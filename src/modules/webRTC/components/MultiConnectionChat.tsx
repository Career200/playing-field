...
export const WebRTCChat = () => {
  ...
  const [actualPeerConnection, setActualPeerConnection] =
    useState<webRTC_connection>();
  ...
  const channelButtonClick = useCallback(
    (index: number) => {
  ...
  return (
    ...
    >
      ...
      >
        <ChannelButton
          key={"C-0"}
          active={pressedButtons[0]}
          text="All"
          onClick={() => channelButtonClick(0)}
        />
        {peerConnections.map((_, i) => {
          ...
          return (
            <ChannelButton
              key={"C-" + index}
              active={pressedButtons[index]}
              onClick={() => channelButtonClick(index)}
              text={"C-" + i}
            />
          );
        })}
      ...
      >
        ...
        <Box height={30} width="100%" flexDirection="row" gap={10}>
          ...
          <SidebarButton
            ...
            page={page}
            width="100%"
            onClick={newAnswerClick}
            text={"Answer"}
          />
        </Box>

        <Box justifyContent="center" id="user-name">
          {userInfo.name}
        </Box>
